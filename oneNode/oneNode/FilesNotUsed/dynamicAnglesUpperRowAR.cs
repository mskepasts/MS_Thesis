...
namespace oneNode
{
    public class dynamicAnglesUpperRowARComponent : GH_Component
    {
        /// <summary>
        ...
        /// </summary>
        public dynamicAnglesUpperRowARComponent()
            : base("dynamic angles upper Row Relation angles", "DA upper Row AR",
            "Description",
        ...
        protected override void RegisterOutputParams(GH_Component.GH_OutputParamManager pManager)
        {
            ...
            pManager.Register_DoubleParam("Cost1", "Cost1", "Cost1 in order to change the node to fit the required angles");
            pManager.Register_DoubleParam("Cost2", "Cost2", "Cost2 in order to change the node to fit the required angles");
            pManager.Register_DoubleParam("Cost3", "Cost3", "Cost3 in order to change the node to fit the required angles");
            pManager.Register_DoubleParam("Cost4", "Cost4", "Cost4 in order to change the node to fit the required angles");
        ...
        public double FindingSideAngle(Point3d p, Point3d pOther, Plane basePlane)
        {
            ...
            double angle = Vector3d.VectorAngle(pOther - p, basePlane.ClosestPoint(pOther) - p, FindingSidePlane( p,  pOther,  basePlane));
            ...
            List<double> anglesSide, int previousCasting, List<int> currentCastings, int direction, int newCasting)
        {
            ...
            double holderMax = double.MaxValue;
            ...
            for (int j = 0; j < length; j++)
            {
                ...
                if ((holder) < holderMax)
                {
                    currentCastings[newCasting] = j;
                    holderMax = holder;
                }
            }
            return Math.Round(holderMax, 2);
        }
            ...
            for (int i = 0; i < length; i++)
                for (int j = 0; j < length; j++)
                    for (int q = 0; q < length; q++)
                    {
                        ...
                        {
                            holderMax = holder;
                            currentCastings[cast1] = i;
                            currentCastings[cast2] = j;
                            currentCastings[cast3] = q;
                        }
                    }
        ...
        public List<Plane> GetTopPlanes(List<Point3d> pts, Mesh domeMesh)
        {
            ...
            {
                if (domeMesh.ClosestMeshPoint(pt, 5) != null)
                {
                    Vector3d normal = domeMesh.NormalAt(domeMesh.ClosestMeshPoint(pt, 5));
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            DataTree<double> costs1 = new DataTree<double>();
            DataTree<double> costs2 = new DataTree<double>();
            DataTree<double> costs3 = new DataTree<double>();
            DataTree<double> costs4 = new DataTree<double>();
            ...
            for (int i = topLineIndexStart; i < topLines.Count - 1; i++)
            {
                ...
                if (previousCasting_9 == -1)
                {
                    for (int w = 0; w < topAngles1.Count; w++)
                    {
                        for (int e = 0; e < topAngles1.Count; e++)
                        {
                            previousCasting_0 = w;
                            previousCasting_9 = e;

                            for (int j = 0; j < topAngles1.Count; j++)
                            {
                                for (int q = 0; q < topAngles1.Count; q++)
                                {
                                    ...
                                    // need to check this!!!
                                    currentCastings[6] = castingIndex[branch_current + 1];
                                    currentCastings[7] = castingIndex[branch_current];

                                    double topAngle1 = GetTopAngle(topAngles1, topAngles2, currentCastings[6], currentCastings[5]);
                                    double topAngle2 = GetTopAngle(topAngles1, topAngles2, j, currentCastings[7]);

                                    holderPoint = FindXandYandZ(topAngle1, sideAngles[q], topAngle2, planes[1], planes[3], p);
                                    ...
                                    if (holderPoint.DistanceTo(p[0]) < value1)
                                    {
                                        if (Cost(topAngles1, topAngles2, currentCastings, sideAngles, planes, p, holderPoint, ref otherCost) < value2)
                                        {
                                            ...
                                            if (holderCost < cost)
                                            {
                                                ...
                                                newPoint.Add(holderPoint);
                                                newPoint.Add(p[1]);
                                                newPoint.Add(p[2]);
                                                newPoint.Add(p[3]);
                                                List<double> anglesT = FindAnglesTop(newPoint, planes);
                                                List<double> anglesS = FindAnglesSide(newPoint, planes);
                                                List<double> anglesTop1 = topAngles1;
                                                List<double> anglesTop2 = topAngles2;
                                                List<double> anglesSide = sideAngles;

                                                cost1 = FindMinMaxBoth_2Members(anglesT[0], anglesS[1], anglesS[0], topAngles1, anglesTop2, anglesSide, currentCastings[0], currentCastings, 1, 1);
                                                cost2 = FindMinMaxBoth_2Members(anglesT[3], anglesS[5], anglesS[6], anglesTop1, anglesTop2, anglesSide, currentCastings[6], currentCastings, 0, 5);
                                                cost3 = FindMinMaxBoth_3Members(new double[] { anglesT[1], anglesT[2] }, new double[] { anglesS[2], anglesS[3], anglesS[4] }, anglesTop1, anglesTop2, anglesSide, currentCastings, 2, 3, 4);
                                                cost4 = FindMinMaxBoth_3MembersBTM(new double[] { anglesT[4], anglesT[5] }, new double[] { anglesS[8], anglesS[7], anglesS[9] }, anglesTop1, anglesTop2, anglesSide, currentCastings, 8);
                ...
                else
                {
                    ...
                    for (int j = 0; j < topAngles1.Count; j++)
                    {
                        for (int q = 0; q < topAngles1.Count; q++)
                        {
                            ...
                            currentCastings[7] = castingIndex[branch_current];
                            double topAngle1 = GetTopAngle(topAngles1, topAngles2, currentCastings[6], currentCastings[5]);
                            double topAngle2 = GetTopAngle(topAngles1, topAngles2, previousCasting_9, j);
                ...
                if (bestFit.Count < 1)
                    for(int j = 0; j < currentCastings.Count; j++)
                        bestFit.Add(0);
                ...
                if (cost == double.PositiveInfinity)
                {
                    ...
                    costs1.Add(dist * 300, pth);
                    costs2.Add(dist * 300, pth);
                    costs3.Add(dist * 300, pth);
                    costs4.Add(dist * 300, pth);
                ...
                else
                {
                    ...
                    costs.Add(cost, pth);
                    costs1.Add(cost1, pth);
                    costs2.Add(cost2, pth);
                    costs3.Add(cost3, pth);
                    costs4.Add(cost4, pth);
            ...
            DA.SetDataTree(7, costs1);
            DA.SetDataTree(8, costs2);
            DA.SetDataTree(9, costs3);
            DA.SetDataTree(10, costs4);
            DA.SetDataTree(11, angleCosts);
            DA.SetDataTree(12, sidePlanes);
            DA.SetDataTree(13, topPlanes);
            DA.SetDataTree(14, meshes);
            DA.SetDataTree(15, castings);
        ...
        /// </summary>
        public override Guid ComponentGuid => new Guid("fe5ced54-92a9-493c-a09a-7d3e71000ec4");
    }
}