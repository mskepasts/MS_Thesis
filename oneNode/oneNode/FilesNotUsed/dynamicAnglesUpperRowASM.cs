...
namespace oneNode
{
    public class dynamicAnglesUpperRowASMComponent : GH_Component
    {
        /// <summary>
        ...
        /// </summary>
        public dynamicAnglesUpperRowASMComponent()
            : base("dynamic angles upper Row Static angles multi", "DA upper Row ASM",
            "Description",
        ...
        public double Cost(List<double> anglesTop1, List<double> anglesTop2, List<int> currentCastings, List<double> anglesSide, List<Plane> planes, List<Point3d> p, Point3d pointFound, ref double angleCost)
        {
            ...
            FindMinMaxBoth_2Members(anglesT[3], anglesS[5], anglesS[6], anglesTop1, anglesTop2, anglesSide, currentCastings[6], currentCastings, 0, 5);
            FindMinMaxBoth_3Members(new double[] { anglesT[1], anglesT[2] }, new double[] { anglesS[2], anglesS[3], anglesS[4] }, anglesTop1, anglesTop2, anglesSide, currentCastings, 2, 3, 4);
            FindMinMaxBoth_3MembersBTM(new double[] { anglesT[4], anglesT[5] }, new double[] { anglesS[8], anglesS[7], anglesS[9] }, anglesTop1, anglesTop2, anglesSide, currentCastings, 8);


            double minCostDist = pointFound.DistanceTo(p[0]);
            double scaleAngles = 6.0;
            double scaleDist = 4.0;
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            for (int i = topLineIndexStart; i < topLines.Count - 1; i++)
            {
                ...
                foreach (int casting8 in getcasting(sideAngles, 8))
                {
                    foreach (int casting5 in getcasting(sideAngles, 5))
                    { 
                    currentCastings[8] = casting8;
                    currentCastings[5] = casting5;
        ...
        /// </summary>
        public override Guid ComponentGuid => new Guid("9b834a9c-b691-4477-86d3-d04e696700ef");
    }
}